...
export const SVG_ICONS: Record<string, string> = {
    ...
    'sub-workflow': `<svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
        <rect x="3" y="3" width="18" height="18" rx="2" fill="#009688" opacity="0.2"/>
        <path d="M19 3 H5 C3.9 3 3 3.9 3 5 V19 C3 20.1 3.9 21 5 21 H19 C20.1 21 21 20.1 21 19 V5 C21 3.9 20.1 3 19 3 Z M19 19 H5 V5 H19 V19 Z M11 7 h2 v2 h-2 z M11 11 h2 v2 h-2 z M11 15 h2 v2 h-2 z" fill="#009688"/>
    </svg>`,
    ...
    'webhook-trigger': `<svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
        ...
        <path d="M7.5 10.5 L11 14 L17 8" stroke="#FF5722" stroke-width="2.5" fill="none" stroke-linecap="round"/>
        <circle cx="12" cy="12" r="8" stroke="#FF5722" stroke-width="2" fill="none"/>
    </svg>`
};

/**
 * Get icon SVG by BlockType
 */
export function getBlockIcon(type: BlockType): string {
    const iconKey = type.toString();
    return SVG_ICONS[iconKey] || SVG_ICONS['start'];
}

/**
 * Get icon SVG by icon name
 */
export function getIconSvg(iconName: string): string {
    return SVG_ICONS[iconName] || SVG_ICONS['start'];
}
