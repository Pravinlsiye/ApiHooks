...
export type IconType = 'start' | 'end' | 'http' | 'variable' | 'condition' | 'delay' | 'log' | 'evaluate' | 'loop' | 'trycatch';

/**
 * SVG icon definitions
 * All icons are 24x24 viewBox for consistency
 */
export const SVG_ICONS: Record<IconType, string> = {
    start: `<svg viewBox="0 0 24 24" fill="currentColor">
        <circle cx="12" cy="12" r="10" fill="#4CAF50"/>
        <path d="M9 8 L16 12 L9 16 Z" fill="white"/>
    </svg>`,
    
    end: `<svg viewBox="0 0 24 24" fill="currentColor">
        <circle cx="12" cy="12" r="10" fill="#f44336"/>
        <rect x="8" y="8" width="8" height="8" rx="1" fill="white"/>
    </svg>`,
    
    http: `<svg viewBox="0 0 24 24" fill="currentColor">
    ...
    variable: `<svg viewBox="0 0 24 24" fill="currentColor">
    ...
    condition: `<svg viewBox="0 0 24 24" fill="currentColor">
    ...
    delay: `<svg viewBox="0 0 24 24" fill="currentColor">
    ...
    log: `<svg viewBox="0 0 24 24" fill="currentColor">
    ...
    evaluate: `<svg viewBox="0 0 24 24" fill="currentColor">
    ...
    loop: `<svg viewBox="0 0 24 24" fill="currentColor">
    ...
    trycatch: `<svg viewBox="0 0 24 24" fill="currentColor">
 ...
 */
export function createIcon(type: IconType, className?: string, size: number = 24): HTMLElement {
    const container = document.createElement('span');
    container.className = `icon icon-${type}${className ? ' ' + className : ''}`;
    container.style.display = 'inline-block';
    container.style.width = `${size}px`;
    container.style.height = `${size}px`;
    container.innerHTML = SVG_ICONS[type];
 ...
 */
export const LEGACY_EMOJI_ICONS: Record<IconType, string> = {
    start: '🟢',
    end: '🔴',
    http: '🌐',
    variable: '📦',
