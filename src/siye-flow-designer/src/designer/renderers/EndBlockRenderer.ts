...
export class EndBlockRenderer extends BlockRenderer {
    ...
    constructor(block: VisualBlock, blockData?: EndBlock) {
    ...
    public updatePorts(): void {
        const endBlock = this.blockData as EndBlock;
        if (!endBlock || !endBlock.config) return;
        ...
        Object.keys(effectiveOutputs).forEach((outputName, portIndex) => {
            ...
            const portY = this.calculatePortY(portIndex, 0);
     ...
     */
    private getEffectiveOutputs(endBlock: EndBlock): Record<string, any> {
        return endBlock.config?.outputs || {};
    }
    ...
    protected renderPorts(): string {
        ...
        return outputEntries.map(([outputName, outputDef]) => {
